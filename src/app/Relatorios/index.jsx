...
export default function RelatorioPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const totalFinanceiro = relatorioTotal.reduce((acc, r) => acc + r.totalReceber, 0);
    const somaAlunos = relatorioTotal.reduce((acc, r) => acc + (r.mediaAlunos * r.aulasRealizadas), 0);
    const mediaAlunos = totalRealizadas > 0 ? Math.round(somaAlunos / totalRealizadas) : 0;
    const custoMedio = relatorioTotal.length > 0 ? relatorioTotal.reduce((acc, r) => acc + r.valorHora, 0) / relatorioTotal.length : 0;
    ...
    <div className="p-6 md:p-8 max-w-[1920px] mx-auto animate-fade-in space-y-8">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700 text-xs">
              ...
              {relatorioVisivel.length === 0 && <tr><td colSpan="10" className="p-8 text-center text-slate-400 text-sm">Sem dados encontrados.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {itensVisiveis < relatorioTotal.length && (
          <div className="flex flex-wrap justify-center gap-3 pb-4 animate-fade-in">
              ...
              >
                  <DownloadCloud className="w-4 h-4"/> Ver Todos ({relatorioTotal.length})
              </button>
          </div>
      )}
    </div>
